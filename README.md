...
- Every target builds in CI; only Web has been run by a person. Signing
  (Android release key, Apple Developer ID and notarization, iOS
  provisioning), the Play 200 MB check, and a first launch on each platform
  are open. Per-target state: `AGENTS.md`, "Targets".
