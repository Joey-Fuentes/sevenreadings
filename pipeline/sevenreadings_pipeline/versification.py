"""Masoretic (MT) numbering -> canonical (English) verse ids.
...
from .refs import BY_OSIS, decode, verse_id
...
def apply_mt(
    ...
    for v in verses:
        if v.book == PSALMS:
            off = offsets.get(v.chapter, 0)
            ch, n = v.chapter, (0 if v.verse <= off else v.verse - off)
        else:
