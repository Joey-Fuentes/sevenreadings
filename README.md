...
(Academic; Internet Archive OCR, unproofread). All public domain or CC BY /
CC BY-SA; see `docs/licensing.md`. Ibn Kathir remains a documented stub
(`pipeline/sevenreadings_pipeline/sources/stubs.py`).
...
- One of seven commentary parsers: Ibn Kathir has no English text with a
  shippable license and no Bible-Qur'an parallels data yet. The ICC has one
  volume; more are one `sources.toml` entry each.
